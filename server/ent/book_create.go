...
func (bc *BookCreate) AddRefereeIDs(ids ...int) *BookCreate {
	bc.mutation.AddRefereeIDs(ids...)
	return bc
}

// AddReferees adds the "referees" edges to the Book entity.
func (bc *BookCreate) AddReferees(b ...*Book) *BookCreate {
	ids := make([]int, len(b))
	for i := range b {
		ids[i] = b[i].ID
	}
	return bc.AddRefereeIDs(ids...)
}

// AddReferenceIDs adds the "references" edge to the Book entity by IDs.
func (bc *BookCreate) AddReferenceIDs(ids ...int) *BookCreate {
	bc.mutation.AddReferenceIDs(ids...)
	return bc
}

// AddReferences adds the "references" edges to the Book entity.
func (bc *BookCreate) AddReferences(b ...*Book) *BookCreate {
	ids := make([]int, len(b))
	for i := range b {
		ids[i] = b[i].ID
	}
	return bc.AddReferenceIDs(ids...)
}
...
func (bc *BookCreate) createSpec() (*Book, *sqlgraph.CreateSpec) {
	...
	if nodes := bc.mutation.RefereesIDs(); len(nodes) > 0 {
	...
	if nodes := bc.mutation.ReferencesIDs(); len(nodes) > 0 {
		edge := &sqlgraph.EdgeSpec{
			Rel:     sqlgraph.M2M,
			Inverse: false,
