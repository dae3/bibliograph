...
type BookQuery struct {
	...
	withReferees   *BookQuery
	withReferences *BookQuery
...
func (bq *BookQuery) QueryReferees() *BookQuery {
	...
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		...
		step := sqlgraph.NewStep(
			...
			sqlgraph.Edge(sqlgraph.M2M, true, book.RefereesTable, book.RefereesPrimaryKey...),
		)
...
func (bq *BookQuery) QueryReferences() *BookQuery {
	query := (&BookClient{config: bq.config}).Query()
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		if err := bq.prepareQuery(ctx); err != nil {
			return nil, err
		...
		step := sqlgraph.NewStep(
			sqlgraph.From(book.Table, book.FieldID, selector),
			sqlgraph.To(book.Table, book.FieldID),
			sqlgraph.Edge(sqlgraph.M2M, false, book.ReferencesTable, book.ReferencesPrimaryKey...),
		)
		fromU = sqlgraph.SetNeighbors(bq.driver.Dialect(), step)
		return fromU, nil
	}
...
func (bq *BookQuery) Clone() *BookQuery {
	...
	return &BookQuery{
		...
		withReferees:   bq.withReferees.Clone(),
		withReferences: bq.withReferences.Clone(),
...
func (bq *BookQuery) WithReferees(opts ...func(*BookQuery)) *BookQuery {
	query := (&BookClient{config: bq.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
	bq.withReferees = query
	return bq
}
...
func (bq *BookQuery) WithReferences(opts ...func(*BookQuery)) *BookQuery {
	query := (&BookClient{config: bq.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
	bq.withReferences = query
	return bq
}
...
func (bq *BookQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Book, error) {
	...
	if query := bq.withReferees; query != nil {
		if err := bq.loadReferees(ctx, query, nodes,
			func(n *Book) { n.Edges.Referees = []*Book{} },
			func(n *Book, e *Book) { n.Edges.Referees = append(n.Edges.Referees, e) }); err != nil {
			return nil, err
		}
	}
	if query := bq.withReferences; query != nil {
		if err := bq.loadReferences(ctx, query, nodes,
			func(n *Book) { n.Edges.References = []*Book{} },
			func(n *Book, e *Book) { n.Edges.References = append(n.Edges.References, e) }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (bq *BookQuery) loadReferees(ctx context.Context, query *BookQuery, nodes []*Book, init func(*Book), assign func(*Book, *Book)) error {
	...
	query.Where(func(s *sql.Selector) {
		joinT := sql.Table(book.RefereesTable)
		s.Join(joinT).On(s.C(book.FieldID), joinT.C(book.RefereesPrimaryKey[0]))
		s.Where(sql.InValues(joinT.C(book.RefereesPrimaryKey[1]), edgeIDs...))
		columns := s.SelectedColumns()
		s.Select(joinT.C(book.RefereesPrimaryKey[1]))
	...
	for _, n := range neighbors {
		nodes, ok := nids[n.ID]
		if !ok {
			return fmt.Errorf(`unexpected "referees" node returned %v`, n.ID)
		}
		for kn := range nodes {
			assign(kn, n)
		}
...
func (bq *BookQuery) loadReferences(ctx context.Context, query *BookQuery, nodes []*Book, init func(*Book), assign func(*Book, *Book)) error {
	edgeIDs := make([]driver.Value, len(nodes))
	byID := make(map[int]*Book)
	nids := make(map[int]map[*Book]struct{})
	for i, node := range nodes {
	...
	query.Where(func(s *sql.Selector) {
		joinT := sql.Table(book.ReferencesTable)
		s.Join(joinT).On(s.C(book.FieldID), joinT.C(book.ReferencesPrimaryKey[1]))
		s.Where(sql.InValues(joinT.C(book.ReferencesPrimaryKey[0]), edgeIDs...))
		columns := s.SelectedColumns()
		s.Select(joinT.C(book.ReferencesPrimaryKey[0]))
		s.AppendSelect(columns...)
		s.SetDistinct(false)
	})
	if err := query.prepareQuery(ctx); err != nil {
