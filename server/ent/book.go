...
type BookEdges struct {
	...
	Referees []*Book `json:"referees,omitempty"`
...
func (e BookEdges) RefereesOrErr() ([]*Book, error) {
	if e.loadedTypes[0] {
...
// was not loaded in eager-loading.
func (e BookEdges) ReferencesOrErr() ([]*Book, error) {
	if e.loadedTypes[1] {
...
func (*Book) scanValues(columns []string) ([]any, error) {
	...
	for i := range columns {
		...
		case book.FieldAuthor, book.FieldTitle:
...
// QueryReferees queries the "referees" edge of the Book entity.
func (b *Book) QueryReferees() *BookQuery {
	return NewBookClient(b.config).QueryReferees(b)
}

// QueryReferences queries the "references" edge of the Book entity.
func (b *Book) QueryReferences() *BookQuery {
	return NewBookClient(b.config).QueryReferences(b)
}
